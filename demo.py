...
def resolve_device(device_arg: str) -> torch.device:
    ...
    try:
        ...
        raise ValueError(
            f"Unrecognized device argument '{device_arg}'. "
            "Use 'cpu', 'cuda:<index>', or the numeric index."
        ) from exc
...
def recognize_font(image):
    ...
    with torch.no_grad():
        ...
        available_fonts = len(font_list)
        logits = output[0][:available_fonts]
        prob = logits.softmax(dim=0)

        topk = min(9, available_fonts)
        indicies = torch.topk(prob, topk)[1]

        predictions = {font_list[i].path: float(prob[i]) for i in range(available_fonts)}

        image_updates = [
            gr.update(value=font_demo_images[indicies[i]]) for i in range(topk)
        ]
        label_updates = [
            gr.update(value=f"**Font Name**: {font_list[indicies[i]].path}")
            for i in range(topk)
        ]

        # pad outputs to fixed length (so Gradio output list length stays constant)
        while len(image_updates) < 9:
            image_updates.append(gr.update(value=None))
        while len(label_updates) < 9:
            label_updates.append(gr.update(value="**Font Name**: N/A"))
