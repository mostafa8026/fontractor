    ...
    def generate_line(self, length: int, font: DSFont) -> str:
        ...
        if length > self.when_length_greater_than:
            if random.random() < self.prob:
                ...
                start_place = random.randint(
                    0, length - len(random_english_word))
                ret = (
...
class FarsiRandomCorpusGenerator(CommonCorpusGenerator):
    PLACEHOLDER_CHARSET = (
        "اآأإبپتثجچحخدذرزژسشصضطظعغفقکگلمنوهی"
        "ءئؤئى"
        "۱۲۳۴۵۶۷۸۹۰"
        " ،؛؟ "
    )
    ...
    def __init__(self, avg_chars_per_word: float = 3.5):
        self.avg_chars_per_word = max(1.0, avg_chars_per_word)
        if not FarsiRandomCorpusGenerator._words_ready:
            load_words_from_db()
            FarsiRandomCorpusGenerator._words_ready = True

    def _random_place_holder(self, font: DSFont) -> str:
        try:
            # print(f"Generating placeholder for font {font.path}")
            return random_char(1, font, self.PLACEHOLDER_CHARSET)
        except UnqualifiedFontException:
            return " "

    def _filter_for_font(self, text: str, font: DSFont) -> str:
        filtered = []
        # print(f"Filtering text for font {font.path}")
        for char in text.replace("\n", " "):
            if char_in_font(char, font.path):
                filtered.append(char)
            else:
                filtered.append(self._random_place_holder(font))
        return "".join(filtered)

    def _word_count_for_length(self, remaining_length: int) -> int:
        return max(1, math.ceil(remaining_length / self.avg_chars_per_word))

    def _generate_seed_line(self, target_length: int) -> str:
        words_needed = self._word_count_for_length(target_length)
        return get_random_persian_paragraph(words_needed, 1).replace("\n", " ").strip()

    def generate_line(self, length: int, font: DSFont) -> str:
        if length <= 0:
            return ""

        line = self._filter_for_font(self._generate_seed_line(length), font)

        while len(line) < length:
            extra_words = self._word_count_for_length(length - len(line))
            # print(
                # f"Adding up to {extra_words} extra words to reach length {length}")
            extra_chunk = get_random_persian_word(extra_words).strip()
            # print(f"Generated extra chunk: '{extra_chunk}'")
            if extra_chunk:
                line += self._filter_for_font(" " + extra_chunk, font)
            else:
                break
